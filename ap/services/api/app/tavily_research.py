...
# Serper locale: Taiwan Traditional Chinese news.
SERPER_GL = "tw"
SERPER_HL = "zh-tw"
...
async def _search_serper_news(query: str, start_date: str, end_date: str, num: int = 10, skip_date_filter: bool = False) -> list[dict]:
    ...
    # Pre-filter: parse Serper dates and reject results outside range + 非中文
    filtered = []
    non_cjk_rejected = 0
    for r in raw_results:
        title = r.get("title", "")
        snippet = r.get("snippet", "")
        # TW 情境：reject 完全不含 CJK 的文章（可能誤混入英文外電）。
        # 僅當 title 與 snippet 都無任何 CJK 字元時才丟棄 —— 混語系文章保留。
        if not (_has_cjk(title) or _has_cjk(snippet)):
            logger.info(f"  ✗ Non-CJK filter rejected: '{title[:60]}'")
            non_cjk_rejected += 1
            continue
        raw_date = r.get("date", "")
    ...
    if rejected > 0:
        logger.info(f"Serper news '{query[:30]}': {len(raw_results)} raw → {len(filtered)} after filters ({non_cjk_rejected} non-CJK, {rejected - non_cjk_rejected} date)")
    else:
