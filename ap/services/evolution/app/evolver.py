...
#
# Design notes:
#   - Prefer 3-char full names or name+title combos; single-surname aliases
#     are risky in 中文 (e.g. 「賴」collides 賴清德 / 賴瑞隆 / 賴品妤).
#   - New candidates on 2028 / 直轄市長 templates can be added here — the
#     augment-party-detection path also picks them up from the template.
_CANDIDATE_ALIASES: dict[str, str] = {
    # ── 2024 總統大選 ──
    "賴清德": "賴清德", "賴總統": "賴清德", "總統賴清德": "賴清德", "賴清德總統": "賴清德",
    "侯友宜": "侯友宜", "侯市長": "侯友宜", "新北侯市長": "侯友宜", "侯友宜市長": "侯友宜",
    "柯文哲": "柯文哲", "柯主席": "柯文哲", "柯 P": "柯文哲", "柯p": "柯文哲", "阿北": "柯文哲",
    # ── 2028 民調 / 2028 總統 ──
    "蕭美琴": "蕭美琴", "蕭副": "蕭美琴", "副總統蕭美琴": "蕭美琴",
    "黃國昌": "黃國昌", "黃委員": "黃國昌", "黃主席": "黃國昌",
    "盧秀燕": "盧秀燕", "盧市長": "盧秀燕", "台中盧市長": "盧秀燕", "臺中盧市長": "盧秀燕",
    "蔣萬安": "蔣萬安", "蔣市長": "蔣萬安", "台北蔣市長": "蔣萬安", "臺北蔣市長": "蔣萬安",
    "韓國瑜": "韓國瑜", "韓院長": "韓國瑜", "韓總": "韓國瑜", "立法院長韓國瑜": "韓國瑜",
    "鄭麗文": "鄭麗文", "鄭委員": "鄭麗文",
    # ── 2026 三都市長 ──
    "沈伯洋": "沈伯洋", "Puma": "沈伯洋", "puma": "沈伯洋", "黑熊沈伯洋": "沈伯洋",
    "江啟臣": "江啟臣", "江副院長": "江啟臣",
    "何欣純": "何欣純",
    "麥玉珍": "麥玉珍",
    "柯志恩": "柯志恩", "柯教授": "柯志恩",
    ...
    "民進黨候選人": "民進黨候選人", "綠營候選人": "民進黨候選人",
    "國民黨候選人": "國民黨候選人", "藍營候選人": "國民黨候選人",
    "民眾黨候選人": "民眾黨候選人", "白營候選人": "民眾黨候選人",
}

# Taiwan-context prompts (restored from the Stage 1.9-era TW implementation).
try:
    from .prompts import (
...
# ── Taiwan 5-bucket leaning normaliser ─────────────────────────────────
# Persona rows stored under the US-era code path still use English bucket
# labels ("深綠", "偏藍", "中間"). Normalise on read so the rest
# of the engine can work in 深綠/偏綠/中間/偏藍/深藍 directly.
_TW_BUCKETS = ("深綠", "偏綠", "中間", "偏藍", "深藍")
_BUCKET_NORMALISE = {
    # TW-native (no-op)
    "深綠": "深綠", "偏綠": "偏綠", "中間": "中間", "偏藍": "偏藍", "深藍": "深藍",
    # US→TW
    "深綠": "深綠", "偏綠": "偏綠", "中間": "中間",
    "偏藍": "偏藍", "深藍": "深藍",
    # Legacy CJK 3-tier
    "偏左派": "偏綠", "中立": "中間", "偏右派": "偏藍",
    "強烈左派": "深綠", "強烈右派": "深藍",
    # TPP bucket → 中間 (third force not on blue/green axis)
    "偏白": "中間", "白": "中間",
}


def _tw_bucket(leaning) -> str:
    """Coerce any leaning label into the canonical 5-bucket TW spectrum."""
...
async def evolve_one_day(
        ...
        for d in diaries:
            ...
            if expr in ("高度表達", "highly expressive") or d_anx >= 65:
                social_posts.append({
                    ...
                    "age": _ag.get("age", 40),
                    "leaning": _tw_bucket(d.get("political_leaning", _ag.get("political_leaning", "中間"))),
                    "text": d_text[:150],
                    "anxiety": d_anx,
                })
    ...
    async def _process_agent(agent: dict):
        ...
        if job is not None and job.get("enable_kol"):
            ...
            my_leaning = _tw_bucket(agent.get("political_leaning", "中間"))
            
            def is_in_chamber(kol_ln, my_ln):
                if kol_ln == my_ln: return True
                if _tw_bucket(my_ln) == "中間" or _tw_bucket(kol_ln) == "中間": return True
            ...
            import random
            for tp in trending:
                if isinstance(tp, dict) and is_in_chamber(_tw_bucket(tp.get("leaning", "中間")), my_leaning):
        ...
        # ── Feature 2: Personal Life Event (TW catalog) ──
        life_event = None
        life_event_text = ""
        try:
            from .tw_life_events import TW_EVENT_CATALOG
            from .life_events import roll_life_event
            # Monkey-patch the catalog for TW events
            import importlib
            le_mod = importlib.import_module(".life_events", package="app")
            _orig_catalog = getattr(le_mod, "EVENT_CATALOG", [])
            le_mod.EVENT_CATALOG = TW_EVENT_CATALOG
            ...
            if life_event:
                life_event_text = (
                    ...
                    f"這個事件應該強烈影響你今天的日記與心情。\n"
                )
                _push_live(job, f"🎲 Agent #{aid} 發生事件：{life_event['name']}")
        except Exception as e:
        ...
        my_leaning_social = _tw_bucket(state.get("current_leaning", agent.get("political_leaning", "中間")))
        ...
        current_leaning = _tw_bucket(state.get("current_leaning", agent.get("political_leaning", "中間")))
        political_leaning = current_leaning
        articles_text = _build_articles_text(feed)

        # ── Feature 4: Multi-dimensional political attitudes ──
        # Axes (0-100 scale):
        #   economic_stance   0=左派大政府福利, 100=右派小政府自由市場
        #   social_values     0=進步(婚平/多元), 100=保守(傳統家庭)
        #   cross_strait      0=主權優先/台獨, 100=兩岸整合/統一
        #   issue_priority    自由文字: "主權/民主" / "經濟/兩岸" / "民生"
        attitudes = state.get("political_attitudes")
        if not attitudes:
            # Initialize from coarse 5-bucket leaning (accept both TW and legacy US labels)
            _green_set = ("深綠", "偏綠", "深綠", "偏綠")
            _blue_set  = ("深藍", "偏藍", "深藍", "偏藍")
            if current_leaning in _green_set:
                attitudes = {"economic_stance": 35, "social_values": 30,
                             "cross_strait": 20, "issue_priority": "主權/民主"}
            elif current_leaning in _blue_set:
                attitudes = {"economic_stance": 65, "social_values": 70,
                             "cross_strait": 80, "issue_priority": "經濟/兩岸"}
        ...
        def _att_label_leftright(val: int) -> str:
            """Economic / social-values axis: 0-100 → 左/中/右 labels."""
            if val <= 20: return "強烈左派"
            if val <= 35: return "偏左派"
            if val <= 45: return "中間偏左"
            if val <= 55: return "中立"
            if val <= 65: return "中間偏右"
            if val <= 80: return "偏右派"
            return "強烈右派"

        def _att_label_cross_strait(val: int) -> str:
            """Cross-strait axis: 0=主權派(台獨), 100=統一派(統獨光譜)."""
            if val <= 20: return "強烈主權派(台獨)"
            if val <= 35: return "偏主權派"
            if val <= 45: return "溫和主權派"
            if val <= 55: return "維持現狀中立"
            if val <= 65: return "溫和整合派"
            if val <= 80: return "偏兩岸整合派"
            return "強烈統合派(統一)"

        _econ_label = _att_label_leftright(attitudes.get("economic_stance", 50))
        _social_label = _att_label_leftright(attitudes.get("social_values", 50))
        # Cross-strait: prefer TW key `cross_strait`, fallback to legacy `national_identity`.
        _cross_label = _att_label_cross_strait(
            attitudes.get("cross_strait", attitudes.get("national_identity", 50))
        )
        ...
        if is_ollama:
            ...
            try:
                ...
                if isinstance(e, asyncio.CancelledError):
                    _push_live(job, f"⚠️ Agent #{aid} 被使用者中斷")
                    return
            ...
            # Taiwan ethnicity is the primary identity axis; race/hispanic are legacy fallbacks.
            _ag_ethnicity = (
                agent.get("ethnicity", agent.get("context", {}).get("ethnicity", ""))
                or agent.get("race", agent.get("context", {}).get("race", ""))  # legacy fallback
                or "未提供"
            )
            ...
            _ag_media = agent.get("media_habit", agent.get("context", {}).get("media_habit", "")) or "未提供"
            _ag_issue1 = agent.get("issue_1", agent.get("context", {}).get("issue_1", ""))
            _ag_issue2 = agent.get("issue_2", agent.get("context", {}).get("issue_2", ""))
            _ag_issues = ", ".join(filter(None, [_ag_issue1, _ag_issue2])) or "未特別指定"

            # Build cognitive bias text from individuality (accept both Chinese-native
            # and legacy English bias keys for backward compat with US-era personas)
            _cog_bias = individuality.get("cognitive_bias", "")
            _BIAS_DESC = {
                "樂觀":   "你傾向往好的方面想，即使是負面新聞，也能從中看到正面解讀；景氣差時會覺得「最壞時期已過」，滿意度不容易下降。",
                "悲觀":   "你傾向擔心最壞結果，負面新聞會讓你特別焦慮，好消息則覺得「好景不長」，焦慮值容易上升。",
                "理性":   "你依資料與邏輯判斷，不容易被標題殺人的新聞影響；會讀內容本身。情緒擺盪較小，滿意度與焦慮都變化平緩。",
                "從眾":   "你很在意周遭人怎麼想，身邊都在抱怨時你就跟著抱怨，朋友說還好你就覺得還好；社群貼文對你影響很大。",
                "陰謀論": "你對主流媒體與政府說詞抱持懷疑，常覺得事件背後有隱情；偏好另類資訊來源，看到官方數字會先質疑真實性。",
                "替罪羊": "壞事發生時你會把責任歸給在位者，連個人不順也歸因到政治環境；負面新聞時地方/中央滿意度都會大幅下降。",
                "冷感":   "你對政治新聞基本無感，除非直接衝擊生活（物價、薪水），才會影響情緒；數值變化應該很小。",
                # Legacy English keys — fall through to translated copies
                "optimistic": "你傾向往好的方面想；即使負面新聞也能找到正面解讀，滿意度不容易下降。",
                "pessimistic": "你傾向擔心最壞結果，負面新聞讓你特別焦慮，焦慮值容易上升。",
                "rational": "你依資料與邏輯判斷；情緒擺盪小，滿意度與焦慮都變化平緩。",
                "conformist": "你很在意周遭人的看法，情緒隨群體起伏，社群貼文影響很大。",
                "conspiracy-prone": "你對主流媒體與官方說詞抱持懷疑，常覺得事件背後有隱情。",
                "scapegoating": "壞事歸責在位者；負面新聞讓地方/中央滿意度大幅下降。",
                "apathetic": "對政治新聞基本無感；除非影響生活，否則情緒變化很小。",
            }
            cognitive_bias_text = ""
            if _cog_bias and _cog_bias in _BIAS_DESC:
                cognitive_bias_text = (
                    f"[你的認知偏誤：{_cog_bias}]\n{_BIAS_DESC[_cog_bias]}\n"
                    f"這個偏誤會影響你如何解讀新聞、情緒往哪個方向移動。"
                    f"確保日記與數字變化反映這個偏誤。\n"
                )

            # Generate semantic descriptions for current state (Chinese labels)
            _ls = round(local_satisfaction)
            _ns = round(national_satisfaction)
            _ax = round(anxiety)
            _local_desc = "相當滿意" if _ls >= 70 else "還算滿意" if _ls >= 55 else "普通" if _ls >= 45 else "有些不滿" if _ls >= 30 else "非常不滿"
            _national_desc = "相當滿意" if _ns >= 70 else "還算滿意" if _ns >= 55 else "普通" if _ns >= 45 else "有些不滿" if _ns >= 30 else "非常不滿"
            _anxiety_desc = "高度焦慮" if _ax >= 70 else "中度焦慮" if _ax >= 55 else "平常" if _ax >= 40 else "放鬆"
        ...
        # 3 axes: economic_stance / social_values / cross_strait.
        # Shift values accept left/right (經濟軸), progressive/conservative (社會軸),
        # independence/unification (兩岸軸), plus legacy US enums (inclusive/restrictive)
        # for backward compatibility with US-era personas.
        _shift_map = {
        ...
        # Prefer Taiwan-native `cross_strait_shift`, fall back to legacy `national_identity_shift`.
        ...
        new_attitudes = {
            ...
            # Store under both keys for forward/backward compatibility
            "cross_strait": max(0, min(100, attitudes.get("cross_strait", attitudes.get("national_identity", 50)) + _shift_map.get(_cross_shift_raw, 0))),
        ...
        _is_right = current_leaning in ("偏藍", "深藍")
        _is_left = current_leaning in ("偏綠", "深綠")
        _is_neutral = current_leaning == "中間"
        ...
        if job is not None and "daily_summary" in job:
            ...
            if not current_summary:
                ...
                all_leanings = set(leaning_map.values()) if leaning_map else {"深綠", "偏綠", "中間", "偏藍", "深藍"}
            ...
            for ag in agents:
                ...
                ag_leaning = ag.get("political_leaning", "中間")
                ...
                if cand_names:
                    ...
                    for ci, cname in enumerate(cand_names):
                        ...
                        # Harris.
                        ag_is_solid_dem  = ag_leaning == "深綠"
                        ag_is_lean_dem   = ag_leaning == "偏綠"
                        ag_is_solid_rep  = ag_leaning == "深藍"
                        ag_is_lean_rep   = ag_leaning == "偏藍"
                        ag_is_dem = ag_is_solid_dem or ag_is_lean_dem
                        ag_is_rep = ag_is_solid_rep or ag_is_lean_rep
                        ag_is_tossup = ag_leaning == "中間"
    ...
    if job is not None and job.get("enable_kol"):
        ...
        kol_diaries = []
        agent_leaning_map = {a.get("person_id"): a.get("political_leaning", "中間") for a in agents}
        ...
        for e in day_entries:
            ...
            if aid in kol_ids and e.get("diary_text"):
                kol_diaries.append({
                    ...
                    "leaning": agent_leaning_map.get(aid, "中間")
                })
...
def _push_live(job: dict, msg: str):
    ...
    is_shift = ("shifted from" in msg.lower()) or ("Leaning shift" in msg) or ("政治傾向變動" in msg) or ("轉為中間" in msg) or ("轉為偏綠" in msg) or ("轉為偏藍" in msg) or ("軟化至中間" in msg)
...
def _augment_party_detection(
    ...
    賴清德 and 賴瑞隆; 蔣 matches 蔣萬安 and 蔣渭水).
    """
    ...
    try:
        ...
        for day in range(1, days + 1):
            ...
            for a in agents:
                ...
                agent_leaning_map[aid] = a.get("political_leaning", "中間")
