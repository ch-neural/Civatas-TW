...
def _check_eligibility(event: dict, agent: dict) -> bool:
    ...
    # Ethnicity — Taiwan native field (閩南/客家/外省/原住民/新住民)
    # Required: event only fires if agent matches this ethnicity
    required_eth = elig.get("ethnicity")
    if required_eth is not None:
        agent_eth = agent.get("ethnicity", agent.get("race", ""))
        if agent_eth != required_eth:
            return False

    # Excluded ethnicities (event must NOT fire for these)
    excluded_eths = elig.get("ethnicity_not")
    if excluded_eths is not None:
        agent_eth = agent.get("ethnicity", agent.get("race", ""))
        if isinstance(excluded_eths, str):
            excluded_eths = [excluded_eths]
        if agent_eth in excluded_eths:
            return False

    # Legacy US fields — kept for backward compat with US-era personas.
    ...
    # Tenure (自有住宅 / 租屋 / Owner / Renter — TW and US labels both accepted)
    required_tenure = elig.get("tenure")
    if required_tenure is not None:
        agent_tenure = agent.get("tenure", agent.get("household_tenure", ""))
        if required_tenure not in str(agent_tenure) and required_tenure != agent_tenure:
            return False
