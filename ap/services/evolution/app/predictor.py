...
# Prompts come from the TW-native prompts.py. us_predictor_helpers is kept as a
# soft dependency: if it's deleted, we fall back to no-op helpers (TW detection
# logic inside this file covers the common cases; see _get_leaning_for_candidate
# below and _augment_party_detection).
try:
...
try:
    ...
    logger.info("us_predictor_helpers not available; using TW-native fallbacks")
    us_get_leaning = None
    us_detect_party = None
    us_is_incumbent = None
    us_is_admin = None
...
def _get_leaning_for_candidate(candidate_key: str) -> str:
    ...
    Taiwan: match DPP/KMT/TPP substring in the key or candidate name.
    Legacy US helper is still consulted for legacy US-era prediction rows
    (candidate_key format 'Democratic — Jane Doe').

    Example: '民進黨 — 賴清德' → '偏綠'
             '國民黨 — 盧秀燕' → '偏藍'
             '民眾黨 — 黃國昌' → '中間'
    """
    key_str = str(candidate_key or "")
    # TW native detection
    if any(tag in key_str for tag in ("民進黨", "DPP", "綠營")):
        return "偏綠"
    if any(tag in key_str for tag in ("國民黨", "KMT", "藍營")):
        return "偏藍"
    if any(tag in key_str for tag in ("民眾黨", "TPP", "白營", "白色力量")):
        return "中間"
    # Legacy US fallback
    if us_get_leaning is not None:
        try:
            result = us_get_leaning(key_str)
            # Normalise US bucket → TW bucket
            return {
                "Solid Dem": "深綠", "Lean Dem": "偏綠", "Tossup": "中間",
                "Lean Rep": "偏藍", "Solid Rep": "深藍",
            }.get(result, result if result in ("深綠", "偏綠", "中間", "偏藍", "深藍") else "中間")
        except Exception:
            pass
    return "中間"  # fallback if helper unavailable
