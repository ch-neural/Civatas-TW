...
# Soft-fail import: us_news_keywords may have been removed. When absent we fall
# back to TW-native keyword pools so build_default_keywords still works.
try:
    try:
    ...
    US_BUILD_KEYWORDS = None
    US_SERPER_LOCALE = "zh-TW"


_TW_LOCAL_KW_POOL = [
    "縣市長", "市議會", "地方預算", "交通", "捷運", "治安",
    "學校", "健保", "長照", "房價",
]
_TW_NATIONAL_KW_POOL = [
    "總統", "行政院", "立法院", "兩岸", "中國", "國防", "軍購",
    "通膨", "物價", "央行", "能源", "核能", "健保", "勞退",
    "教育", "少子化", "房價",
]


def build_default_keywords(county: str) -> tuple[list[str], list[str]]:
    if US_BUILD_KEYWORDS is not None:
        try:
            return US_BUILD_KEYWORDS(county)
        except Exception:
            pass
    # TW fallback: prefix local pool with county name; national pool as-is
    local = [f"{county} {kw}" for kw in _TW_LOCAL_KW_POOL if county] or list(_TW_LOCAL_KW_POOL)
    national = list(_TW_NATIONAL_KW_POOL)
