...
# TW: Chinese values are canonical so UI and diaries align. The evolver
# `_personality_modifiers` and `_BIAS_DESC` accept both Chinese and English keys.
_PERSONALITY_DIMS = {
    "expressiveness": ["高度表達型", "一般", "內斂"],
    "emotional_stability": ["穩定冷靜", "一般穩定", "敏感衝動"],
    "sociability": ["外向", "中度社交", "內向"],
    "openness": ["開放多元", "中度開放", "固守觀點"],
}
...
def compute_individuality(personality: dict, age: int = 40, occupation: str = "", income_band: str = "") -> dict:
    ...
    if expr in ("highly expressive", "高度表達", "高度表達型"): react_base *= 1.15
    elif expr in ("reserved", "沉默寡言", "內斂"): react_base *= 0.85
    ...
    # cognitive_bias 值必須對應 evolver._BIAS_DESC 的 keys — 同時支援繁中（canonical）
    # 與英文（legacy US fallback）。TW 版本輸出繁中 key。
    bias_pool = []
    if es in ("sensitive / impulsive", "敏感衝動"):
        ...
        bias_pool += ["理性", "樂觀", "從眾"]
    if op in ("set in views", "固守觀點"): bias_pool += ["陰謀論"]
    if op in ("open to new ideas", "開放多元"): bias_pool += ["理性", "樂觀"]
    _ib = (income_band or "").lower()
    if any(k in income_band for k in ("高收入", "中高收入", "12-20萬", "20萬以上")) \
       or any(k in _ib for k in ("$100k", "$150k", "$200k")):
        bias_pool += ["樂觀"]
    if any(k in income_band for k in ("低收入", "中低收入", "3萬以下", "3-5萬")) \
       or any(k in _ib for k in ("under $25k", "$25k", "$50k")):
        bias_pool += ["悲觀", "替罪羊"]
    if age < 30: bias_pool += ["從眾"]
    if age > 60: bias_pool += ["悲觀"]
    cognitive_bias = _rng.choice(bias_pool) if bias_pool else "理性"
