    ...
    <div className="fixed inset-0 bg-[#0a0a1a] z-50 flex flex-col items-center justify-center overflow-y-auto">
      <div className="w-full max-w-2xl px-6 py-12">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-[#e94560] tracking-wider">CIVATAS TW</h1>
                  ...
                  <div className="space-y-4 text-sm">
                    ...
                    <div className="bg-[#16213e] rounded-lg p-4 border border-[#0f3460]">
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        <div>
                          <div className="text-neutral-200 font-medium">
                            {en ? "DGBAS Census + MOI Monthly Report" : "主計總處普查 + 戶政司月報"}
                          </div>
                          <div className="text-neutral-500 text-[11px] mt-0.5">
                            {src?.demographics ?? "110 年人口及住宅普查 + 2024 年底月報 + 客委會/原民會族群調查"}
                          </div>
                        </div>
                      </div>
                      <div className="text-neutral-400 text-xs leading-relaxed ml-5">
                        <p className="mb-1.5">
                          {isState
                            ? (en
                              ? `Covering ${sel?.region ?? "county"}: ${countyCount} townships, population ${fmtPop}.`
                              : `涵蓋${sel?.region ?? "縣市"}：${countyCount} 個鄉鎮市區，人口 ${fmtPop}。`)
                            : (en
                              ? `Covering ${stateCount} counties + ${countyCount} townships, population ${fmtPop}.`
                              : `涵蓋 ${stateCount} 縣市 + ${countyCount} 個鄉鎮市區，人口 ${fmtPop}。`)}
                        </p>
                        <div className="text-neutral-500">
                          <span className="text-neutral-400">{en ? "Dimensions: " : "維度："}</span>
                          {en
                            ? "Gender, Age (7 brackets), Ethnicity (5: 閩南/客家/外省/原住民/新住民), Education (5 levels), Household Income (6 NT$ brackets), Household Type, Employment (3 categories), Housing tenure, Media habit, County, Township, Party lean (5-bucket blue-green)"
                            : "性別、年齡（7 區間）、族群（閩南/客家/外省/原住民/新住民）、教育（5 級）、家戶所得（6 級 NT$）、家戶型態、就業（3 類）、居住、媒體習慣、縣市、鄉鎮、藍綠傾向（5 桶）"}
                        </div>
                        <div className="text-neutral-500 mt-1">
                          <span className="text-neutral-400">{en ? "Sources: " : "資料來源："}</span>
                          主計總處 110 年普查 · 戶政司 · 客委會 2021 · 原民會 2024
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#16213e] rounded-lg p-4 border border-[#0f3460]">
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        <div>
                          <div className="text-neutral-200 font-medium">
                            {en ? "CEC 2024 Presidential Election" : "中選會 2024 總統大選"}
                          </div>
                          <div className="text-neutral-500 text-[11px] mt-0.5">
                            {src?.elections ?? "中選會 2024 鄉鎮級開票資料（368 鄉鎮市區）"} · 公開資料
                          </div>
                        </div>
                      </div>
                      <div className="text-neutral-400 text-xs leading-relaxed ml-5">
                        {en
                          ? `Township-level presidential results for ${isState ? sel?.region : "all of Taiwan"} (2024). Used to compute Blue-Green partisan lean.`
                          : `${isState ? sel?.region : "全國"}鄉鎮級總統選舉結果（2024：賴清德 40.05% / 侯友宜 33.49% / 柯文哲 26.46%），用於計算藍綠傾向。`}
                      </div>
                    </div>
                    ...
                    <div className="bg-[#16213e] rounded-lg p-4 border border-[#0f3460]">
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        <div>
                          <div className="text-neutral-200 font-medium">
                            {en ? "Blue-Green Partisan Voting Index (TW-PVI)" : "藍綠傾向指數 (TW-PVI)"}
                          </div>
                          <div className="text-neutral-500 text-[11px] mt-0.5">
                            {src?.leaning ?? "從 2024 兩黨得票率計算，類比 Cook PVI"}
                          </div>
                        </div>
                      </div>
                      <div className="text-neutral-400 text-xs leading-relaxed ml-5">
                        <p className="mb-1.5">
                          {isState
                            ? (en
                              ? `${sel?.region}: ${countyCount} townships, overall ${pviLabel ?? "—"}.`
                              : `${sel?.region}：${countyCount} 個鄉鎮市區，整體 ${pviLabel ?? "—"}。`)
                            : (en
                              ? `${cwl ?? countyCount} townships with computed PVI.`
                              : `${cwl ?? countyCount} 個鄉鎮市區有計算 PVI。`)}
                        </p>
                        <div className="text-neutral-500">
                          <span className="text-neutral-400">{en ? "Formula: " : "公式："}</span>
                          {en
                            ? "township Green 2-party% − national Green% (民進黨 vs 國民黨)"
                            : "鄉鎮綠營兩黨得票率 − 全國綠營兩黨得票率（民進黨 vs 國民黨）"}
                        </div>
                        <div className="text-neutral-500 mt-1">
                        ...
                        <div className="text-neutral-500 mt-1 text-[10px]">
                          {en
                            ? "Third-force (TPP / 民眾黨 柯文哲) tracked separately as white_share — not on the 藍綠 axis."
                            : "第三勢力（民眾黨柯文哲）獨立追蹤 white_share —— 不在藍綠軸上。"}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#16213e] rounded-lg p-4 border border-[#0f3460]">
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <div className="text-neutral-500 text-[11px] mt-0.5">
                            g0v / ronnywang twgeojson · 內政部 MOI segis · MIT 授權
                          </div>
                        </div>
                      </div>
                      <div className="text-neutral-400 text-xs leading-relaxed ml-5">
