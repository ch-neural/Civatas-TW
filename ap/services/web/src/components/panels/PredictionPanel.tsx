...
export default function PredictionPanel({ wsId }: { wsId: string }) {
  ...
  useEffect(() => {
    ...
    // Taiwan has no electoral college — popular vote decides the presidency.
    // Always disable EC; the checkbox is hidden for TW templates via the use_electoral_college
    // flag in the template itself (set to false in build_templates.py).
    setUseElectoralCollege(false);
                    ...
                    {[
                      {
                        ...
                        desc: en
                          ? "Pure popular vote: every citizen counts the same. Taiwan's presidential election is decided purely by popular vote (no electoral college), so this matches the real 中選會 tally method used in 2024 (賴清德 40.05% / 侯友宜 33.49% / 柯文哲 26.46%)."
                          : "純普選票：每個公民一票等重。台灣總統大選純粹由普選決定（無選舉人團），這個模式就對應 2024 中選會的實際計票方式（賴清德 40.05%／侯友宜 33.49%／柯文哲 26.46%）。",
                        use: en
                      ...
                      },
                      {
                        ...
                        desc: en
                          ? "Models actual Taiwan turnout: young (<30) ~60%, middle-aged 70–80%, seniors (60+) ~75%. The 70/30 label is a general modern-pollster shorthand (TVBS/山水/聯合報/菱傳媒 等 hybrid 手機+市話樣本）。"
                          : "對準真實台灣投票率：青年（<30）約 60%、中壯年 70–80%、長者（60+）約 75%。70/30 名稱源自現代民調（TVBS、山水、聯合報、菱傳媒等）常用的混合抽樣法。",
                        use: en
                          ? "Best for: Matching real election results (2024 backtest, 2028 forecast)."
                          : "適合：對準真實選舉結果（2024 回測、2028 預測）。",
                        color: "#22c55e",
                      },
                      {
                        id: "landline_only",
                        ...
                        desc: en
                          ? "Simulates old landline-only polls. Taiwan landlines skew heavily older and traditionally 偏藍, systematically underestimating young / 綠營 / 白色力量 voters. The 2014 柯文哲 當選、2016 蔡英文 大贏 都讓純市話民調跌破眼鏡。"
                          : "模擬純打市話的舊式民調。台灣市話族群嚴重偏向年長與泛藍，系統性低估青年、綠營與白色力量。2014 柯文哲當選、2016 蔡英文大勝都讓純市話民調跌破眼鏡。",
                        use: en
                    ...
                    <label style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: running ? "not-allowed" : "pointer" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, marginTop: 4, lineHeight: 1.6, fontFamily: "var(--font-cjk)" }}>
                          {en
                            ? "Taiwan has NO electoral college — the presidency is decided by popular vote alone. This option is kept disabled for all Taiwan templates. County-level winner-take-all tallies can still be inspected in the per-county breakdown below, but there is no EC threshold to reach."
                            : "台灣沒有選舉人團制度 —— 總統由普選直接決定勝負。台灣 template 一律停用此選項。如需看各縣市勝選情形，可在下方分縣市 breakdown 檢視，但沒有像美國那樣的選舉人門檻。"}
                        </div>
                      </div>
                    </label>
                  </div>
