        ...
        {viewTab === "overview" && (
          <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
            ...
            <div style={{ width: 240, flexShrink: 0, display: "flex", flexDirection: "column", gap: 6 }}>
              {(() => {
                // Map highlight follows the active template's scope (TW: county scope).
                const isCountyScope =
                  (activeTemplate?.election?.scope === "county" || activeTemplate?.election?.scope === "state") &&
                  !!activeTemplate?.region_code;
                const countyKey = isCountyScope ? (activeTemplate.region_code as string) : "";
                const mapTitle = isCountyScope ? (activeTemplate.region || countyKey) : "台灣";
                return (
                  <USMap
                    mode="states"
